...
class BaseItem (object):
    ...
    def onInspectorGUI (self, inspector):
        inspector.addHeader(self.rank, self.name, self.imgPath)
        inspector.addDescription (self.description)
        inspector.addVSpace (5)
        labels = []
        self.collectLabelsForInspector(labels)
        inspector.addLabels(labels)
        inspector.addActionButtons(self)
        
    #for easier rewrite
    def collectLabelsForInspector (self, labels):
        labels.append([StrUtil.ITEM_ATTR_VALUE, fMoneyStr(self.value)])
    
    def onComposerDraw (self, composer, amount, min, max):
        composer.addLabel(tStr(self.name))
        composer.beginHorizontal()
        composer.addImg(self.imgPath)
        composer.endHorizontal()
        if min == max :
            if min == -1:
                composer.addSpinBox(amount, 1, 9999999999)
            else:
                composer.beginHorizontal()
                composer.addSpinBox(amount, 1, max)
                composer.addLabel("/{}".format(max))
                composer.endHorizontal()
        else:
            composer.beginHorizontal()
            composer.addSpinBox(1, max)
            composer.addLabel("({}-{})".format(min, max))
            composer.endHorizontal()
...
class ComposeableItem (BaseItem):
    ...
    def onComposerDraw (self, composer, amount, min, max):
        ...
        if min == max :
            if min == -1:
                composer.addSpinBox(amount, 0, 9999999999)
            else:
                composer.beginHorizontal()
                composer.addSpinBox(amount, 0, 100)
                composer.addLabel("{}".format(max))
                composer.endHorizontal()
        else:
            composer.beginHorizontal()
